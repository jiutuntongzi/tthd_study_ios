...
@implementation NoteDetailCommentCell
...
-(void)setModel:(NoteDetailCommentModel *)model
{
    ...
    if (model.img.length > 0) {
        ...
        [_commentImageView sd_setImageWithURL:[NSURL URLWithString:model.img] placeholderImage:UIImageMake(@"common_icon_placeholderImage")];
    ...
    //回复
    if ([model.reply_number intValue] == 0) {
        _replyButton.sd_layout.heightIs(0);
        _commentButton.sd_layout.topSpaceToView(_commentImageView, 10);
    }else{
        [_replyButton setTitle:[NSString stringWithFormat:@"%@等人共%@条回复", model.reply_nickname, model.reply_number] forState:0];
...
-(void)tapImage
{
    self.tapImageBlock(@[_model.img], 0);
}
