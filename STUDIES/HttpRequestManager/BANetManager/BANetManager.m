                   ...
                   progressBlock:(BAUploadProgressBlock)progressBlock
{
    ...
    [BANetManagerShare.sessionManager POST:fileEntity.urlString parameters:fileEntity.parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        NSData *data = [NSData dataWithContentsOfFile:fileEntity.filePath];
...
////    NSString *outfilePath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:videoWritePath];
//
//
//
//
...
//        NSLog(@"%ld", (long)[avAssetExport status]);
//        if ([avAssetExport status]== 4) {
//            NSLog(@"%@", [avAssetExport error]);
//        }
//        
//        
...
//                    NSURL *filePathURL2 = [NSURL URLWithString:fileEntity.filePath];
//
//                    // 获得沙盒中的视频内容
                             ...
                             index:(NSUInteger)index
{
    ...
    {   // 图片数据不为空才传递 fileName
        ...
        formatter.dateFormat = @"yyyyMMddHHmmss";
        NSString *str = [formatter stringFromDate:[NSDate date]];
        NSString *imageFileName = [NSString stringWithFormat:@"%@%@",str, fileNames[index]];
